...
import { fetchRecipeFromUrl } from '../../utils/recipeImport';
...
const blankForm = () => ({
  name: '', author: '', page: '', servings: '', rating: 0, notes: '', image: '',
...
export function RecipeEditorModal({ show, editingRecipe, initialBookId, books, allTags, onClose, onSave, onDelete }: RecipeEditorModalProps) {
  ...
  useEffect(() => {
    ...
    if (editingRecipe) {
      setForm({
        name: editingRecipe.name, author: editingRecipe.author, page: editingRecipe.page, servings: editingRecipe.servings,
  ...
  const save = async () => {
    ...
    await onSave(bookId || null, { ...form, name: form.name.trim(), ingredients, instructions, sourceUrl: importUrl, author: form.author || '' });
    onClose();
  };
  ...
  return (
    <Overlay show={show} onClose={onClose}>
      ...
      <div className="field">
        <label htmlFor="re-url">Import from a URL</label>
        <div className="tag-input-wrap">
          <input type="text" id="re-url" placeholder="https://cooking.nytimes.com/recipes/..."
            value={importUrl} onChange={(e) => setImportUrl(e.target.value)} />
          <button type="button" className="btn small" onClick={runImport} disabled={importing}>
            {importing ? 'Fetching...' : 'Fetch'}
          </button>
        </div>
        {importError && <p className="hint" style={{ color: 'var(--spine-red)' }}>{importError}</p>}
      </div>
      <div className="field">
      ...
      <div className="field">
        <label htmlFor="re-steps">Steps — one per line</label>
        <textarea id="re-steps" rows={6} value={instructionsText} onChange={(e) => setInstructionsText(e.target.value)} />
      </div>
